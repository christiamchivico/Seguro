...
use App\Models\AcEstatusDetalle;
 ...
 {      
   ...
   if ($user->type == 3){
        $query = DB::table('ac_claves')
                ...
                ->join('ac_estatus_detalle'        , 'ac_estatus_detalle.id','=','ac_claves_detalle.estatus')                
                ...
                ->select('ac_claves_detalle.id as id',
                         ...
                         'ac_estatus_detalle.nombre as nombre_estatus_detalle'
                        );
   }elseif ($user->type == 4) // Analista Aseguradora  
     {
        ...
        //$filter->add('ac_tipo_documentos.id','Seleccione una Opción','select')->option('','Seleccione Una Opción')->options(AcTipoDocumento::lists('descripcion', 'id')->all());
        $filter->add('ac_estatus_detalle.id','Seleccione una opcion ','select')->option('','Seleccione Una Opción')->options(AcEstatusDetalle::lists('nombre', 'id')->all());         
       ...
       $grid->add('nombre_estatus_detalle','Estatus', false);
       $grid->add('aseguradora','Aseguradora', false);
       $grid->add('colectivo','Colectivo', false);
       $grid->add('proveedor','Proveedor', false);
       $grid->addActions('/altocentro/public/auditoria/consultarDetalle', 'Ver','show','id');
        
      if (isset($_GET['export'])){        
            return $grid->buildCSV('auditoria','.Y-m-d.His');
        }else{
            $grid->paginate(10);
            return  view('auditorias.consultarPacienteAtendido', compact('filter','grid'));
    ...
    {        
      ...
      if (isset($id['clave'])){
            ...
            $documentos = DB::table('ac_documentos')
             ...
             return view('auditorias.consultarDetalle', compact('clave','clave_detalle', 'pacienteatendido','documentos'));
         }
     }    
     
    protected function download(Request $request)
    {
        $doc = AcDocumento::findOrFail($request->id_documento);
        $nombre = $doc->file;
