...
//    }
 ...
 public function procesarRegistrar(Request $request)
 {
   ...
   public  function subirArchivo($clave, $cedula_afiliado,$nombre_archivo){ 
            ...
            Session::flash('message', 'Cantidad Máxima de Archivo deben se cinco(5).');
             return false;
                ...
                for($i = 0; $i < count($archivos); $i++):    
                   ...
                   if($FileType != "doc" && $FileType != "docs" && $FileType != "jpg" && $FileType != "pdf" && $FileType != "png" &&
                      $FileType != "DOC" && $FileType != "DOCS" && $FileType != "JPG" && $FileType != "PDF" && $FileType != "PNG" &&
    ...
    {
        $cartaAval = AcDocumento::where('id','=', $request->input('id_documento'))->first();
