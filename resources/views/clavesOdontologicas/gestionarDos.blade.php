...
@section('content')
    ...
    @if (isset($claves) and (count($claves) > 0))
        {!! Form::open(['url' => 'clavesOdonto/gestionarTres', 'class' => 'form-horizontal', 'name' => 'beneficiario']) !!}
                                ...
                                <tr>
                                    ...
                                    <td>{!! Form::hidden('fecha_atencion1'.$x, $clave->fecha_atencion1) !!}
                                        @if ($clave->acTipoControl->id == 3)
                                            {{ Carbon\Carbon::parse($clave->fecha_atencion1)->format('m-Y') }}
                                        @else
            ...
            <div class="col-sm-3 col-sm-offset-4" id="btnSecuencia" hidden="true">
                {!! Form::submit('Gestionar Secuencia', ['class' => 'btn btn-primary form-control', 'id' => 'seleccionar', 'disabled' => 'disabled', 'hidden' => 'true']) !!}
            </div>
            <div class="col-sm-3 col-sm-offset-4">
                <a href="#" class="btn btn-primary form-control" id="showClaves">Gestionar Secuencia</a>
            </div>
        {!! Form::close() !!}
    @else
    <div class="col-sm-12">
...
@section('script')
    ...
    <script>
        $(function(){
            ...
            $('input:radio[name=iclave]').on('click',function(){
                $('#seleccionar').prop('disabled', false);
            });

            $('#showClaves').on('click', function(){
                $('#clavesActivas').show();
                $('#btnSecuencia').show();
                $('#showClaves').hide();
